...
export class RealtimeConversation {
	defaultFrequency = 8_000; // 8,000 Hz
	...
	EventProcessors = {
		'conversation.item.created': (event) => {
			...
			newItem.formatted = {};
			newItem.formatted.audio = Array.from(new Uint8Array(0));
			...
			if (newItem.type === 'message') {
				if (newItem.role === 'user') {
					...
					if (this.queuedInputAudio) {
						newItem.formatted.audio = Array.from(this.queuedInputAudio);
		...
		'conversation.item.truncated': (event) => {
			...
			item.formatted.audio = Array.from(item.formatted.audio.slice(0, endIndex));
		...
		'response.audio.delta': (event) => {
			...
			const appendValues = new Uint8Array(arrayBuffer);
			item.formatted.audio = Array.from(RealtimeUtils.mergeUint8Arrays(
