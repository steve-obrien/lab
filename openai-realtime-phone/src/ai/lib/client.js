...
export class RealtimeClient extends RealtimeEventHandler {
	...
	_resetConfig() {
		...
		this.inputAudioBuffer = new Uint8Array(0);
		return true;
	}
	...
	_addAPIEventHandlers() {
		...
		this.realtime.on('server.input_audio_buffer.speech_started', (event) => {
			console.log('speech_started', Date.now(), event)
			handler(event);
	 ...
	 * @returns {true}
	 */
	appendInputAudioBase64(base64) {
		if (base64.length > 0) {
			this.realtime.send('input_audio_buffer.append', {
				audio: base64,
			});
			const arrayBuffer = new Uint8Array(RealtimeUtils.base64ToArrayBuffer(base64));
			this.inputAudioBuffer = RealtimeUtils.mergeUint8Arrays(
