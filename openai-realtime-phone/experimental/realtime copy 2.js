...
fastify.register(async (fastify) => {
	fastify.get('/media-stream', { websocket: true }, async (twilioWs, req) => {
			...
			try {
				...
				switch (data.event) {
					case 'media':
						if (client.realtime.isConnected()) {
							...
							if (item) {
								...
								// Throttle the webLog.send call to once every 1000ms
								if (!item.lastSent || Date.now() - item.lastSent > 1000) {
									webLog.send(item);
									item.lastSent = Date.now();
								}
							}
							client.appendInputAudioBase64(data.media.payload);
							// add to buffer
					...
					case 'start':
						...
						console.log('Tel: Incoming stream has started', data, streamSid);
						break;
					case 'connected':
						...
						break;
					case 'dtmf':
						console.log('Tel: Received dtmf:', data.dtmf.digit);
						break;
					default:
						console.log('Tel: Received non-media event:', data.event);
						break;
				}
			} catch (error) {
		...
		client.realtime.on('server.session.created', (event) => {
			console.log('AI: session created', event)
		})
		client.realtime.on('server.session.updated', (event) => {
			console.log('AI: session updated', event)
		})
		...
		client.realtime.on('server.conversation.item.created', (event) => {
			console.log('AI: server.conversation.item.created', event.item.id)
...
const logWsClients = new Map(); // Map to store WebSocket connections by call ID
