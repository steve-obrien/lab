const { app, BrowserWindow, ipcMain, desktopCapturerm, dialog } = require('electron');
...
function startRecording() {
	...
	recordingWindow = new BrowserWindow({
		...
		webPreferences: {
			nodeIntegration: false,
			contextIsolation: true,
			preload: __dirname + '/recorder-preload.js',
			enableRemoteModule: false
		}
	});

	recordingWindow.loadFile('recorder.html');

	recordingWindow.on('closed', () => {
		recordingWindow = null;
		// Enable "Start Recording", disable "Stop Recording"
		updateTrayMenu();
	});

	// Enable/Disable tray menu items
	updateTrayMenu(true);
}
...
// Function to update tray menu items
function updateTrayMenu(isRecording = false) {
	const contextMenu = Menu.buildFromTemplate([
		// ... existing menu items ...
		{ type: 'separator' },
		{ label: 'Start Recording', click: startRecording, enabled: !isRecording },
		{ label: 'Stop Recording', click: stopRecording, enabled: isRecording },
		{
			label: 'Open Cam', click: () => {
				createWindow();
			}
